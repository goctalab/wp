...
class rise_action extends WP_Widget {



// constructor

    function rise_action() {

		$widget_ops = array('classname' => 'rise_action_widget', 'description' => esc_html__( 'Create a call-to-action for a home page widget area.', 'rise_widgets') );

        parent::__construct(false, $name = esc_html__('MT - Call-to-Action', 'rise_widgets'), $widget_ops);  

		$this->alt_option_name = 'rise_action_widget';
	...
	// Check values
		...
		$action_text 		= isset( $instance['action_text'] ) ? sanitize_text_field( $instance['action_text'] ) : '';

		$action_btn_link 	= isset( $instance['action_btn_link'] ) ? sanitize_text_field( $instance['action_btn_link'] ) : '';

		$action_btn_text 	= isset( $instance['action_btn_text'] ) ? sanitize_text_field( $instance['action_btn_text'] ) : '';
	...
	<label for="<?php echo sanitize_text_field( $this->get_field_id('action_text')); ?>"><?php esc_html_e('Enter your call to action.', 'rise_widgets'); ?></label>
	...
	<label for="<?php echo sanitize_text_field( $this->get_field_id('action_btn_link')); ?>"><?php esc_html_e('Link for the button', 'rise_widgets'); ?></label>
	...
	<label for="<?php echo sanitize_text_field( $this->get_field_id('action_btn_text')); ?>"><?php esc_html_e('Title for the button', 'rise_widgets'); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		$instance['action_btn_link'] = sanitize_text_field($new_instance['action_btn_link']);
		...
		if ( isset($alloptions['rise_action']) )

			delete_option('rise_action');		  
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('rise_action', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'rise_action', 'widget' );
		...
		$title 			 = apply_filters( 'widget_title', sanitize_text_field( $title ), $instance, $this->id_base );
...
?>
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'rise_action', $cache, 'widget' ); 
